...
setInterval(() => {
  http.get(`https://kb-system.glitch.me`);
}, 280000);
...
const youtube = new YouTube("AIzaSyAEMI3GSXPwuIJiAFN4FJn-Q_scEwu-Kvk"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyAEMI3GSXPwuIJiAFN4FJn-Q_scEwu-Kvk"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${prefix}help / https://discord.gg/9vFk68h`);
});
...
\`قناة مصمم البوت\` : <https://cutt.us/bVBZq>
\`رابط سيرفر المساعده\` : https://discord.gg/Ec7z7aH
\`رابط ادخال البوت لسيرفرك\` : <https://cutt.us/HqpTX>
`);
        })
        .then(e => {
          message.react("✅");
...
client.on("message", message => {
  if (message.content === "invite") {
    message.channel.send("انفايت بوت السستم : https://cutt.us/KRp8r");
    message.channel.send("انفايت بوت البرودكاست : https://cutt.us/UmaXu");
    message.channel.sendFile("");
  }
});
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    const wlcImage = await fsn.readFile("./welcome99.png"); //اسم الصورة
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "setMessage")) {
    if (!welcome[message.guild.id] || !welcome[message.guild.id].onoff == "off")
      return message.channel.send(
